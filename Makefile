...
define CMDS
ifeq ($(1), runserver)
	envdir envs/$(ENV) dsa_merchant/manage.py$(1)$(PORT)
else
$(1):
	envdir envs/$(ENV) dsa_merchant/manage.py$(1)
endif
endef
...
create-db:
	envdir envs/$(ENV) createdb -U dsa-merchant -l en_US.utf-8 -E utf-8 -O dsa-merchant -T template0 -e dsa-merchant

create-db-user:
	psql -d postgres -c "CREATE USER \"dsa-merchant\" WITH PASSWORD 'dsa-merchant' CREATEDB;"

drop-db:
	envdir envs/$(ENV) dropdb -i -e -U dsa-merchant dsa-merchant

drop-db-user:
	dropuser -i -e dsa-merchant
...
isort:
	isort --recursive setup.py dsa_merchant/ tests/
...
startapp:
	...
	mkdir -p dsa_merchant/apps/$$app_name; \
	envdir envs/$(ENV) python manage.py startapp $$app_name dsa_merchant/apps/$$app_name --template dsa_merchant/config/app_template; \
	echo "Don't forget to add 'dsa_merchant.apps."$$app_name".apps."$$app_name_title"Config' to INSTALLED_APPS in 'dsa_merchant.config/settings/common.py'!"
...
test-upload:
	...
	python -c "import webbrowser; webbrowser.open('https://testpypi.python.org/pypi/dsa_merchant')"

upload:
	twine upload -s dist/*
	python -c "import webbrowser; webbrowser.open('https://pypi.python.org/pypi/dsa_merchant')"
